...
const SnapshotManagerContent: React.FC<SnapshotManagerProps> = ({
  ...
  const handleSaveEdit = (snapshotId: string) => {
    try {
      ...
      if (editLabel.trim() !== '') {
        updatedTimeline = TimelineService.renameSnapshot(snapshotId, editLabel, updatedTimeline);
      }
      ...
      new Notice(
        `Failed to update snapshot: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
  ...
  const handleCreateADR = async (snapshot: TimelineSnapshot) => {
    ...
    try {
      ...
      const updatedTimeline = adrService.linkADRToSnapshot(snapshot.id, adrPath, timeline);
      onTimelineUpdate(updatedTimeline);
  ...
  const handleLinkADR = async (snapshot: TimelineSnapshot) => {
    ...
    const adrPath = prompt('Enter path to ADR markdown file:', 'ADRs/0001-example-decision.md');
  ...
  const handleUnlinkADR = (snapshot: TimelineSnapshot) => {
    ...
    try {
      const updatedTimeline = adrService.unlinkADRFromSnapshot(snapshot.id, timeline);
      onTimelineUpdate(updatedTimeline);
      ...
      new Notice(
        `Failed to unlink ADR: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
  ...
  return (
    <div style={{ padding: SPACING.padding.panel }}>
      ...
      >
        {timeline.snapshots.length} snapshot{timeline.snapshots.length !== 1 ? 's' : ''} in timeline
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING.gap.normal }}>
        {orderedSnapshots.map((snapshot, index) => {
          ...
          const editHint = isCurrent
            ? 'Currently editing this snapshot'
            : 'Click to edit this snapshot';
                  ...
                  {snapshot.adrPath ? (
                    ...
                    >
                      <div style={{ marginBottom: SPACING.gap.tiny }}>
                        💡 Link an Architecture Decision Record (ADR) to document why this snapshot
                        exists.
                      </div>
                      <div style={{ display: 'flex', gap: SPACING.gap.tiny }}>
                        <button
                  ...
                  <div style={{ display: 'flex', gap: SPACING.gap.normal }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: timeline.snapshots.length === 1 ? UI_COLORS.textMuted : '#FF6B6B',
                        cursor: timeline.snapshots.length === 1 ? 'not-allowed' : 'pointer',
