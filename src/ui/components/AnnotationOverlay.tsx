...
interface AnnotationOverlayProps {
	...
	onAnnotationTextUpdate: (id: string, text: string, formatting?: { fontSize?: number; bold?: boolean; underline?: boolean }) => void;
...
const StickyNoteAnnotation: React.FC<{
	...
	onDragEnd: () => void;
	onTextUpdate: (text: string, formatting?: { fontSize?: number; bold?: boolean; underline?: boolean }) => void;
...
const parseMarkdown = (text: string): React.ReactNode[] => {
	...
	lines.forEach((line, lineIndex) => {
		...
		if (line.startsWith('# ')) {
			...
			elements.push(
				<div key={lineIndex} style={{ fontSize: '18px', fontWeight: 'bold', marginTop: '8px', marginBottom: '4px' }}>
		...
		// Handle headers (## Header)
			...
			elements.push(
				<div key={lineIndex} style={{ fontSize: '16px', fontWeight: 'bold', marginTop: '6px', marginBottom: '3px' }}>
					{parseInlineMarkdown(headerText)}
				</div>
			);
		}
...
const TextBoxAnnotation: React.FC<{
	...
	const [resizeStart, setResizeStart] = React.useState({ width: 0, height: 0, mouseX: 0, mouseY: 0 });
		...
		>
			{annotations.map((annotation) => {
				...
				const commonProps = {
					...
					onTextUpdate: (text: string, formatting?: { fontSize?: number; bold?: boolean; underline?: boolean }) =>
