...
export class ChangeDetectionService {
  ...
  static compareSnapshots(
    ...
    // Nodes
    const addedNodes = afterSnapshot.nodes.filter((n) => !beforeNodeIds.has(n.id)).map((n) => n.id);

    const removedNodes = beforeSnapshot.nodes
      .filter((n) => !afterNodeIds.has(n.id))
      .map((n) => n.id);

    const modifiedNodes = this.detectModifiedNodes(beforeSnapshot.nodes, afterSnapshot.nodes);
    ...
    // Edges
    const addedEdges = afterSnapshot.edges.filter((e) => !beforeEdgeIds.has(e.id)).map((e) => e.id);
   ...
   */
  static setNodeChangeIndicator(node: Node, indicator: ChangeIndicator, changeNote?: string): Node {
    return {
      ...node,
      data: {
        ...node.data,
